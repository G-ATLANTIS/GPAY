...
(() => {
  ...
  const challenge = store.issue({ ttl_ms: 30000, now: NOW });
  assert.match(challenge.nonce_sha256, /^[0-9a-f]{64}$/);
  assert.equal(store.verify().issued_count, 1);
  assert.equal(store.assertUsable({ nonce_sha256: challenge.nonce_sha256, now: NOW + 1000 }).issue_record_sha256, challenge.issue_record_sha256);

  const consumed = store.consume({ nonce_sha256: challenge.nonce_sha256, observation_sha256: H('a'), now: NOW + 1000 });
  assert.match(consumed.consume_record_sha256, /^[0-9a-f]{64}$/);
  assert.equal(store.verify().consumed_count, 1);
  assert.throws(() => store.assertUsable({ nonce_sha256: challenge.nonce_sha256, now: NOW + 1000 }), /ha_runtime_challenge_replay/);
  assert.throws(() => store.consume({ nonce_sha256: challenge.nonce_sha256, observation_sha256: H('a'), now: NOW + 1000 }), /ha_runtime_challenge_replay/);
})();

(() => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'g-bank-ha-challenge-exp-v2-'));
  const store = new HARuntimeChallengeStore(path.join(root, 'challenges.jsonl'));
  const challenge = store.issue({ ttl_ms: 5000, now: NOW });
  assert.throws(() => store.assertUsable({ nonce_sha256: challenge.nonce_sha256, now: NOW + 5000 }), /ha_runtime_challenge_expired/);
  assert.throws(() => store.consume({ nonce_sha256: challenge.nonce_sha256, observation_sha256: H('b'), now: NOW + 5000 }), /ha_runtime_challenge_expired/);
})();
...
(() => {
  ...
  rows[0].expires_at = new Date(NOW + 45000).toISOString();
...
(() => {
  ...
  const store = new HARuntimeChallengeStore(path.join(root, 'challenges.jsonl'));
  assert.throws(() => store.assertUsable({ nonce_sha256: H('c'), now: NOW }), /ha_runtime_challenge_not_issued/);
})();

console.log('G-BANK sovereign v2 HA runtime challenge tests: PASS');
