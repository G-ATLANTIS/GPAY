...
function fixture() {
  ...
  const env = {
    ...
    G_BANK_TREASURY_ASSESSMENT_SHA256: H('7'), G_BANK_CUSTOMER_MONITORING_AUDIT_SHA256: H('8'),
    G_BANK_RECOVERY_AUDIT_SHA256: H('9'),
  };
  ...
  const evidence_bindings = {
    legal_authorization_evidence_sha256: env.G_BANK_LEGAL_AUTHORIZATION_EVIDENCE_SHA256,
    scheme_participation_evidence_sha256: env.G_BANK_SCHEME_PARTICIPATION_EVIDENCE_SHA256,
    settlement_access_evidence_sha256: env.G_BANK_SETTLEMENT_ACCESS_EVIDENCE_SHA256,
    production_identity_evidence_sha256: env.G_BANK_PRODUCTION_IDENTITY_EVIDENCE_SHA256,
  ...
  const readiness = {
    ...
    recovery_checkpoint_state_root_sha256: H('d'), ha_checkpoint_state_root_sha256: H('d'),
    ha_voter_journal_root_sha256: H('f'), ha_cluster_authority_root_sha256: H('e'), ha_fence_valid_until: fence,
    transport_scheme: 'SCT_INST', settlement_system: 'TEST', value_movement_permitted_by_readiness: true,
  };
  const certificate = createTechnicalPromotionCertificate({
    ...
    trusted_signing_key_binding_sha256: H('a'),
  ...
  return { root, env, haState, certificate, runtimeObserver };
}

(async () => {
  const f = fixture();
  const paymentA = {
    message: { message_type: 'pacs.008.001.08', document: '<A/>', document_sha256: H('1') },
    instruction: { instruction_sha256: H('2'), scheme: 'SCT_INST' }, idempotencyKey: 'payment-A',
  };
  const paymentB = { ...paymentA, instruction: { ...paymentA.instruction, instruction_sha256: H('3') }, idempotencyKey: 'payment-B' };
  const operationA = settlementOperationBinding({
    message_sha256: paymentA.message.document_sha256, instruction_sha256: paymentA.instruction.instruction_sha256,
    idempotency_key: paymentA.idempotencyKey, promotion_certificate_sha256: f.certificate.certificate_sha256,
  });
  const witness = issueSyntheticRuntimeHAWitness({
    root: f.root, env: f.env, haAudit: f.haState.haAudit, haDeploymentAudit: f.haState.haDeploymentAudit,
    runtimeObserver: f.runtimeObserver, operation_binding_sha256: operationA.operation_binding_sha256, now: NOW,
  });
  let submits = 0;
  const transport = {
    ...
    async submit(request) { submits += 1; assert.equal(request.settlement_operation_binding_sha256, operationA.operation_binding_sha256); return { submission_id: 'SUB-A', status: 'SUBMITTED', external_receipt_sha256: H('b') }; },
    async readback() { return { status: 'ACCEPTED', external_receipt_sha256: H('c') }; },
  };
  const adapter = new DirectSettlementAdapter({ transport, env: f.env, clock: () => NOW + 1000 });
  ...
  assert.equal(submits, 0, 'cross-payment witness swap must block before transport');
  assert.equal(new HARuntimeChallengeStore(witness.challenge_store_path).verify().consumed_count, 0, 'mismatched operation must not burn the valid challenge');
  ...
  assert.equal(receipt.ha_runtime_challenge_operation_binding_sha256, operationA.operation_binding_sha256);
  assert.equal(new HARuntimeChallengeStore(witness.challenge_store_path).verify().consumed_count, 1);

  console.log('G-BANK sovereign v2 promotion-pinned settlement operation-binding boundary tests: PASS');
