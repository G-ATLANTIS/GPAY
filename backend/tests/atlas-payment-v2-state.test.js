...
s = transition(s, 'PROVIDER_EXECUTED', { provider_execution_receipt_sha256: h('executed') });
assert.equal(s.value_moved, false);
assert.equal(s.creditor_settlement_confirmed, false);
s = transition(s, 'CREDITOR_SETTLEMENT_CONFIRMED', {
  creditor_settlement_receipt_sha256: h('creditor-settlement'),
  creditor_confirmation_verified: true,
  amount_in_minor: 29_490_000,
  currency: 'EUR'
});
assert.equal(s.value_moved, true);
assert.equal(s.creditor_settlement_confirmed, true);
s = transition(s, 'RECONCILED', { reconciliation_receipt_sha256: h('reconciled') });
...
assert.throws(() => transition(
  ...
  {
    creditor_settlement_receipt_sha256: h('x'),
    creditor_confirmation_verified: true,
    amount_in_minor: 100,
    currency: 'EUR'
  }
...
mismatch = transition(mismatch, 'PAYMENT_CREATED_AWAITING_SCA', {
  provider_create_receipt_sha256: h('5'), payment_id: 'p'
});
mismatch = transition(mismatch, 'USER_SCA_AUTHORIZED', { sca_authorization_sha256: h('6') });
mismatch = transition(mismatch, 'PROVIDER_EXECUTED', { provider_execution_receipt_sha256: h('7') });
assert.throws(() => transition(mismatch, 'CREDITOR_SETTLEMENT_CONFIRMED', {
  creditor_settlement_receipt_sha256: h('8'),
  creditor_confirmation_verified: true,
  amount_in_minor: 99,
  currency: 'EUR'
}), /creditor_settlement_amount_mismatch/);

assert.throws(() => transition(mismatch, 'CREDITOR_SETTLEMENT_CONFIRMED', {
  creditor_settlement_receipt_sha256: h('8'),
  creditor_confirmation_verified: false,
  amount_in_minor: 100,
  currency: 'EUR'
}), /creditor_confirmation_not_verified/);
