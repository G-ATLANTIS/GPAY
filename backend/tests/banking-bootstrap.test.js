...
function keyRegex(key, valuePattern) {
  return new RegExp('^' + key + '=' + valuePattern + '$', 'm');
}
...
try {
  ...
  assert.match(envText, keyRegex(operatorSecretKey, '[0-9a-f]{64}'));
  assert.match(envText, keyRegex(probeSecretKey, '[0-9a-f]{64}'));
  assert.match(envText, /^TRUELAYER_PRIVATE_KEY_B64=[A-Za-z0-9+/=]+$/m);
  assert.match(envText, /^TRUELAYER_CLIENT_ID=$/m);
  assert.match(envText, keyRegex(clientSecretKey, ''));
  ...
  const opBefore = envBefore.match(keyRegex(operatorSecretKey, '(.+)'))[1];
  const opAfter = envAfter.match(keyRegex(operatorSecretKey, '(.+)'))[1];
  const probeBefore = envBefore.match(keyRegex(probeSecretKey, '(.+)'))[1];
  const probeAfter = envAfter.match(keyRegex(probeSecretKey, '(.+)'))[1];
